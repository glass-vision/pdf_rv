...
from app.services.credit_memo_importer import ImportResult
...
def mark_upload_job_done(conn: Connection, job_id: str, result: ImportResult) -> None:
    conn.execute(
        ...
        (
            result.total_pages,
            result.total_pages,
            result.credit_memo_count,
            json.dumps(result.credit_memo_numbers),
...
def request_upload_job_cancel(conn: Connection, job_id: str, client_id: str = "") -> bool:
    ...
    Returns True if a job was flagged, False if it does not exist or has
    already finished.
    """
...
def fail_stale_credit_memo_upload_jobs(conn: Connection) -> None:
