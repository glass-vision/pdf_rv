...
from app.services.invoice_extractor import normalize_ref
from app.services.invoice_importer import import_invoice_pdf
...
from app.services.invoice_upload_jobs import (
    UploadCancelled,
    create_upload_job,
    get_upload_job,
...
INVOICE_DATE_DISPLAY_SQL = (
    "CASE "
    "WHEN i.invoice_date GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]' "
    "THEN substr(i.invoice_date, 9, 2) || '/' || substr(i.invoice_date, 6, 2) || '/' || substr(i.invoice_date, 1, 4) "
    "ELSE i.invoice_date "
    "END"
)
...
REF_TYPE_ORDER_SQL = (
    ...
    "WHEN 'sales_order_no' THEN 20 "
    "WHEN 'ext_doc_no' THEN 30 "
    "ELSE 999 END"
)
...
def _process_upload_job(job_id: str, temp_path: Path) -> None:
    try:
        ...
        with db_session() as conn:
            result = import_invoice_pdf(conn, pdf_bytes, on_page=on_page)
...
async def upload_invoices(
    ...
    with db_session() as conn:
        create_upload_job(conn, job_id, file.filename, client_id, "invoices")
...
def cancel_upload_job(
    ...
    request_upload_task_cancel("invoices", job_id)
...
def search_invoices(
    ...
    with db_session() as conn:
        rows = [dict(row) for row in search_document_roots(
            conn,
            doc_type="invoices",
            root_key=normalize_ref(invoice_number) if invoice_number else None,
        ...
        for row in rows:
            row["invoice_number"] = row.pop("root_key")
            iso = row.get("root_date")
            row["invoice_date_iso"] = iso
            row["invoice_date"] = (
...
@router.get("/{invoice_number}/pdf")
def get_invoice_pdf(invoice_number: str):
    with db_session() as conn:
        row = fetch_document_pdf(conn, "invoices", normalize_ref(invoice_number))
    ...
    headers = {"Content-Disposition": f'inline; filename="{normalize_ref(invoice_number)}.pdf"'}
...
def export_invoices_pdf(
    ...
    with db_session() as conn:
        for invoice in invoices:
            row = fetch_document_pdf(conn, "invoices", invoice["invoice_number"])
            if not row:
                raise HTTPException(status_code=404, detail="Invoice No. not found")
            pdf_parts.append(read_pdf_from_storage(row["assembled_pdf"], row["assembled_pdf_path"]))

    output_pdf = assemble_pdf(pdf_parts)
    headers = {"Content-Disposition": 'inline; filename="invoices_export.pdf"'}
